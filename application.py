...
# Predicting on Logistic
y_pred_lr = lr.predict(test_data)
y_pred_lr

# Initializing the Train Scores and Test Scores
print("Train and Test Scores of Logistic Regression:")
train_scores_lr = []
test_scores_lr = []
...
for i in np.linspace(0.01, 10, 10):
    ...
    lr.fit(train_data, train_target)
    train_scores_lr.append(lr.score(train_data, train_target))
    test_scores_lr.append(lr.score(test_data, test_target))

print("Train Scores:",train_scores_lr)
print("Test Scores:",test_scores_lr)

round_train_scores_lr = [round(num, 2) for num in train_scores_lr]
print("Train Scores (Rounded)",round_train_scores_lr)

round_test_scores_lr = [round(num, 2) for num in test_scores_lr]
print("Test Scores (Rounded)",round_test_scores_lr)

# Initializing the Error Rate
print("Error Rate of Logistic Regression:")
error_rate_lr = []
...
for i in np.linspace(0.01, 10, 10):
    ...
    error_rate_lr.append(np.mean(y_pred_lr  != y_test))

print("Error Rate:",error_rate_lr)

round_error_rate_lr = [round(num, 2) for num in error_rate_lr]
print("Error Rate (Rounded):",round_error_rate_lr)

# Visualizing the Train and Test Scores
c_values = np.linspace(0.01, 10, 10)

plt.figure(figsize=(10, 6))
plt.plot(c_values, round_train_scores_lr, marker='o', label='Training Scores', color='blue')
plt.xlabel('C Values')
plt.ylabel('Training Scores')
plt.title('Training Scores vs Regularization Parameter (C)')
plt.legend()
train_scores_lr_img = encode_plot_to_base64()
plt.close()

plt.figure(figsize=(10, 6))
plt.plot(c_values, round_test_scores_lr, marker='o', label='Training Scores', color='blue')
plt.xlabel('C Values')
plt.ylabel('Training Scores')
plt.title('Training Scores vs Regularization Parameter (C)')
plt.legend()
test_scores_lr_img = encode_plot_to_base64()
plt.close()

# Visualizing the Error Rate
plt.figure(figsize=(10,6))
plt.plot(c_values, error_rate_lr, marker='o', color='red', label='Error Rate')
plt.xlabel('C Values')
plt.ylabel('Error Rate')
plt.title('Error Rate vs Regularization Parameter (C)')
plt.legend()
error_rate_lr_img = encode_plot_to_base64()
plt.close()

# Evaluating the Metrics of Logistic Regression
print("\nLogistic Regression")
accuracy_lr = accuracy_score(test_target, y_pred_lr)
print("\nAccuracy:", round(accuracy_lr*100),"%")

print("\nClassification Report:")
class_rprt_lr = classification_report(y_pred_lr, test_target)
print(class_rprt_lr)

formatted_report_lr = format_classification_report(class_rprt_lr)

print("\nConfusion Matrix:")
conf_matrix_lr = confusion_matrix(y_pred_lr, test_target)
plt.figure(figsize=(10,7))
sns.heatmap(conf_matrix_lr, annot=True, fmt='d', cmap='Blues', xticklabels=labels, yticklabels=labels)
confusion_matrix_lr_img = encode_plot_to_base64()
plt.close()
...
plt.figure(figsize=(10,6))
plt.plot(range(1, 21))
plt.plot(range(1, 21), rounded_error_rate_svm, marker='o', label='Error Rate', color='green', linestyle='dotted')
...
# Navigating from Results to Logistic Regression
@application.route('/lr')
def lr():
    global train_scores_lr_img, test_scores_lr_img, error_rate_lr_img, confusion_matrix_lr_img
    render_template(
        'lr.html',
        round_train_scores_lr = round_train_scores_lr,
        round_test_scores_lr = round_test_scores_lr,
        round_error_rate_lr = round_error_rate_lr,
        train_scores_lr_img = train_scores_lr_img,
        test_scores_lr_img = test_scores_lr_img,
        error_rate_lr_img = error_rate_lr_img,
        confusion_matrix_lr_img = confusion_matrix_lr_img,
        accuracy = accuracy_lr,
        formatted_report_lr = formatted_report_lr
    )
